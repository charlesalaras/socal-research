...
int numLayers = 1; // num_layers
int hiddenSize = 311; // output size
...
int main() {
   ...
   // Host variables
    float * x = (float*)malloc(miniBatch * timeSteps * input_size * sizeof(float));
    ...
    cudaErrCheck(cudaMalloc(&T_ptr, 6 * hiddenSize * hiddenSize * sizeof(float)));
    cudaErrCheck(cudaMalloc(&bias_ptr, 5 * hiddenSize + 5 * hiddenSize * sizeof(float)));
    ...
    float * T = (float*)malloc(6 * hiddenSize * hiddenSize * sizeof(float));
    float * bias = (float*)malloc(5 * hiddenSize + (5 * hiddenSize) * sizeof(float));
    float * dropout;
    ...
    cudaErrCheck(cudaMemcpy(T_ptr, T, 6 * hiddenSize * hiddenSize * sizeof(float), cudaMemcpyHostToDevice));
    cudaErrCheck(cudaMemcpy(bias_ptr, bias, 5 * hiddenSize + 5 * hiddenSize * sizeof(float), cudaMemcpyHostToDevice));
